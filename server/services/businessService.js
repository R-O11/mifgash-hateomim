    ...
    try {
      ...
      if (closureRows.length > 0) {
        ...
        if (closure.open_time && closure.close_time) {
           ...
           return { isOpen: !!timeRows[0].isOpenNow, mode: 'special_hours', ...settings };
        }
      }

      // 3. Fallback to regular hours
      // In MySQL, DAYOFWEEK(CURRENT_DATE) returns 1=Sunday...7=Saturday
      ...
      const isOpen = !!isOpenResult[0].isWithinLimits;
